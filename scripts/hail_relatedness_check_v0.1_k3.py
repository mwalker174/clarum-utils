...
2026-07-22 (CLARUM fork):
- pc_relate k=10 -> k=3. On the CLARUM_WGS_JOINT_620 cohort (620 samples,
  family-clustered, ~17.8k Somalier-panel sites), k=10 PCs exceeds the true
  ancestry structure (hwe_normalized_pca eigenvalues: PC1=26.4, PC2=18.6,
  PC3=6.78, then a flat ~2.8-4.0 floor from PC4 on -- that floor is
  family/relatedness structure, not ancestry). Regressing on 10 PCs let
  pc_relate soak up real relatedness as if it were population structure,
  collapsing known duplicate/sibling kinship toward ~0. k=2 and k=3 agree
  to 4 decimal places on all ground-truth pairs and give correct duplicate
  (kin~0.5) / unrelated (kin~0) signatures; k=3 kept as the more
  conservative of the two (captures the one eigenvalue still clearly above
  the noise floor).
'''
###
...
if presaved_kinship_ht_uri!='NA':
    ...
    if score_table=='false':
        rel = hl.pc_relate(mt.GT, 0.01, k=3)
    else:
